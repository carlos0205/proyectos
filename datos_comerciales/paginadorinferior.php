...
$consulta = db_query($query_limit_registros) or die(mysql_error());
$row_registros = mysqli_fetch_assoc($consulta);

if (isset($_GET['totalRows_registros'])) {
  $totalRows_registros = $_GET['totalRows_registros'];
}else{
  $all_registros = db_query($query_registros);
  $totalRows_registros = mysqli_num_rows($all_registros);
}
