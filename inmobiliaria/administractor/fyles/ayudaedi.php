...
$qryreg = "SELECT a.*, i.nomidi FROM ayudavisitante AS a INNER JOIN idipub AS i ON a.codidi = i.codidi WHERE codayuda = '$cod' ";
...
{
auditoria($_SESSION["enlineaadm"],'Ayuda',$cod,'4');
actualizar("ayudavisitante",2,$cod,"codayuda","ayuda.php");
}
...
{
auditoria($_SESSION["enlineaadm"],'Ayuda',$cod,'4');
actualizar("ayudavisitante",2,$cod,"codayuda","ayudaedi.php?cod=$cod&acc=1");
} 
          ...
          <td valign="top"><table width="58%" height="339" border="0" cellpadding="0" cellspacing="0" bgcolor="#F5F5F5" class="marcotabla" style="width: 100%">
            ...
            <tr>
              ...
              <td valign="top"><p>Ayuda
                <input name="txt2nomayudasi" type="text" id="txt2nomayudasi" size="50" value = "<?php  echo $filreg["nomayuda"];?>"maxlength="100" title="Nombre ayuda" />
              </p></td>
...
$oFCKeditor->Value = html_entity_decode( $filreg["desayuda"] ) ;
$oFCKeditor->Create() ;
?></td>
              <td></td>
            </tr>
