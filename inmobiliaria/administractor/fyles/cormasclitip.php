  ...
  <tr>
	...
	if (isset($_POST['enviarno'])){	
		...
		{	
			...
			if(!empty($_POST["chkspam"])){
				$spam = $_POST["chkspam"];
			}else{
				$spam = "1";
			}
			if($spam == 2){
				$qryter = "SELECT emater FROM tercli WHERE envpro = '2' AND codtipter='$tipter' AND codidi = '$idi' UNION SELECT emaspam as email FROM spam WHERE codtipter='$tipter' ";
			}else{
				$qryter = "SELECT emater FROM tercli WHERE envpro = '2' AND codtipter='$tipter' AND codidi = '$idi'";
			}
	...
	<!--DWLayoutTable-->
	<tr>
	<td width="1390" height="52" valign="top" class="titulos"><img src="../images/correo.png" width="48" height="48" align="absmiddle" /> Correo masivo por tipo de cliente [Envia] </td>
	</tr>
	</table></td>
	</tr>
	<tr>
	...
	<tr>
	<td height="34"></td>
	<td valign="top" ><p>Asunto del Mensaje</p></td>
	<td colspan="2" valign="top"><input name="txtasu" type="text" id="txtasu" size="40" value = "<?php if (isset($_POST['txtnom'])) echo $_POST['txtnom']; ?>"maxlength="50" /></td>
	<td rowspan="4" align="right" valign="middle"><div id="correo"  align="right"></div></td>
	<td>&nbsp;</td>
	</tr>
	<tr>
	<td height="23"></td>
	<td valign="top"  <?php if($filpar["tipcli"] == 1){?> style=" visibility:hidden" <?php }?>>Tipo de Cliente                </td>
	<td colspan="2" valign="top"><span  <?php if($filpar["tipcli"] == 1){?> style=" visibility:hidden" <?php }?>>
	<select name="seltip" id="seltip">
	<?
	if (isset($_POST['seltip'])){
		$tip=$_POST['seltip'];
		$qrytip = "SELECT codtipter, nomtipter FROM tipterdet WHERE codtipter <> '$tip' AND codidi = 1 ORDER BY nomtipter ";
		$qrytip1 = "SELECT codtipter, nomtipter FROM tipterdet  WHERE codtipter = '$tip' AND codidi = 1";
		...
		$qrytip = "SELECT codtipter, nomtipter FROM tipterdet WHERE codidi = 1 ORDER BY nomtipter ";
	}
	...
	<tr>
	<td height="28"></td>
	<td colspan="2" valign="top">Idioma para envio de correspondencia
	...
	<td valign="top"><span  <?php if($filpar["cuecla"] == 1){?> style=" visibility:hidden" <?php }?>> Incluir Cuentas sin clasificar <span class="textonegro">
	<input name="chkspam" type="checkbox" id="chkspam" value="2" <?php if(!empty($_POST["chkenvpro"])) { ?>checked="checked"<?php } ?> />
	</span></span></td>
	<td></td>
	</tr>
	<tr>
	<td height="23"></td>
	<td colspan="3" valign="top" class="titulos">Mensaje</td>
	<td></td>
	</tr>
	<tr>
	<td height="182"></td>
	...
	<tr>
	<td height="28"></td>
	<td colspan="4" valign="top"><label>Archivos Adjuntos</span>:</label>
